...
# Logging faqat kritik xatolar uchun
...
# --- USERBOT (XABARLARNI MONITORING QILISH) ---
    ...
    try:
        ...
        if found:
            ...
            report = (f"🔍 <b>Topildi:</b> {', '.join(found)}\n"
                      ...
                      f"<b>👤 Foydalanuvchi:</b> {html.escape(getattr(sender, 'first_name', 'User'))}\n\n"
...
async def delete_menu(callback: types.CallbackQuery, target_prefix=None):
    ...
    kb = [[InlineKeyboardButton(text=f"❌ {item[1]}", callback_data=f"exec_del_{pref_type}_{item[0]}")] for item in data]
...
async def execute_delete(callback: types.CallbackQuery):
    ...
    pref_type, item_id = parts[2], parts[3]
    
    if pref_type == "keyword":
...
# --- QO'SHISH (KETMA-KET VA TO'XTATILMAYDIGAN) ---
...
async def add_start(callback: types.CallbackQuery):
    ...
    txt = "📝 <b>Kalit so'zlarni yuboring:</b>\n(Vergul bilan yoki bittalab)" if prefix == "keyword" else \
          "📡 <b>Guruh havolasini yuboring:</b>\n(@guruh yoki link. Ketma-ket yuboraverishingiz mumkin)"
    
    await callback.message.edit_text(txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Yakunlash va Orqaga", callback_data=f'{prefix}_menu')]
    ]), parse_mode="HTML")
...
async def handle_input(message: types.Message):
    ...
    if state == "wait_keyword":
        ...
        if not links: return await message.answer("❌ Havola xato. Qayta yuboring.")
        
        msg = await message.answer("⏳ Ulanmoqda...")
        success = 0
        for link in links:
            try:
        ...
        await msg.edit_text(f"✅ {success} ta guruh qo'shildi.\n\n📥 <b>Navbatdagi havolani yuboring:</b>", 
